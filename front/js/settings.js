...
async function init() {
    ...
    try {
        ...
        if (!res.ok) {
            ...
            settingsContent.style.justifyContent = 'center';
            settingsContent.innerHTML = `
                <div class="title-editor" style="max-width: 400px; padding: 3rem 2rem;">
                    ...
                    <p style="margin-bottom: 2.5rem; font-size: 0.95rem;">${t('settings.loginDescription')}</p>
                    <a href="/" class="save-btn" style="display: inline-block; text-decoration: none; box-sizing: border-box;">${t('settings.backHome')}</a>
                </div>
            `;
            applyI18n(settingsContent);
            return;
        }
