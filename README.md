...
ログイン不要のデモで、固定データから生成した惑星をすぐに体験できます。

[ログイン不要のデモを開く](https://githubplanet.dev/demo?showcase=typescript)

1. デモを開く
2. 惑星の色・表面・大きさ・回転を見る
3. 実績や言語ごとの表現を確認する
4. css、cpp、go、javascript、rustなどのshowcaseを切り替えて比較する

このデモは実在のGitHubユーザーや本番データベースに依存しません。審査・展示・紹介動画では、まずこのデモで作品の見た目と体験を確認できます。
...
DBや実在ユーザーに依存しない固定テストデータから生成した、言語別のショーケース惑星です。

カードの一覧は[カードギャラリー](docs/cards/README.md)で確認できます。英語版は[Card Gallery](docs/cards/README.en.md)です。
